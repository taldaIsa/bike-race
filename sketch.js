...
function draw() {
  ...
  if (World.frameCount % 150 == 0) {
   ...
   if(CGRosa.isTouching(ciclistaPrincipal)){
    ...
    //Adicione o código para mostrar instruções de reinicialização do jogo em texto aqui
     text("Aperte R para recomeçar",550,200);
 ...
 ciclistaPrincipal.addAnimation("SahilRunning",img2CiclistaPrincipal);
  ...
  //codigo para de reinicializaçao
   if(keyDown("R")){
  estadoJogo = JOGAR
  CGRosa.destroyEach();
  CGAmarelo.destroyEach();
  CGVermelho.destroyEach();   ciclistaPrincipal.addAnimation("SahilRunning", img1CiclistaPrincipal);
  fimdeJogo.visible = false;
